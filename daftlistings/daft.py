...
class Daft:
    ...
    def search(self, fetch_all=True):
        ...
        if len(divs) == 0:
            ...
            if (
                type(self._listing_type) == RentType
                or type(self._listing_type) == CommercialType
            ):
        ...
        while current_page <= total_pages:
            ...
            if len(divs) == 0:
                ...
                if isinstance(self._listing_type, PropertyForRent) or isinstance(
                    self._listing_type, CommercialType
                ):
                    [listings.append(PropertyForRent(div)) for div in divs]
                else:
                    [listings.append(PropertyForSale(div)) for div in divs]
